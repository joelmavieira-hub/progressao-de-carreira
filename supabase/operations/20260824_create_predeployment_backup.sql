-- Server-side checkpoint for the controlled 2026-08-24 career replay deployment.
-- Keeps production data inside the same Supabase project.
BEGIN;
...
CREATE TABLE archive.colaboradores_perfis_20260824t024900z
  (LIKE public.colaboradores_perfis INCLUDING ALL);
INSERT INTO archive.colaboradores_perfis_20260824t024900z
SELECT * FROM public.colaboradores_perfis;

CREATE TABLE archive.colaboradores_20260824t024900z
  (LIKE public.colaboradores INCLUDING ALL);
INSERT INTO archive.colaboradores_20260824t024900z
SELECT * FROM public.colaboradores;

CREATE TABLE archive.career_progression_events_20260824t024900z
  (LIKE public.career_progression_events INCLUDING ALL);
INSERT INTO archive.career_progression_events_20260824t024900z
SELECT * FROM public.career_progression_events;
...
BEGIN
  FOR source_table, backup_table IN VALUES
    ('colaboradores_perfis', 'colaboradores_perfis_20260824t024900z'),
    ('colaboradores', 'colaboradores_20260824t024900z'),
    ('career_progression_events', 'career_progression_events_20260824t024900z')
  LOOP
    EXECUTE format(
      'SELECT count(*), md5(coalesce(string_agg(md5(to_jsonb(t)::text), '''' ORDER BY t.id), '''')) FROM public.%I t',
      source_table
    ) INTO source_count, source_hash;

    EXECUTE format(
      'SELECT count(*), md5(coalesce(string_agg(md5(to_jsonb(t)::text), '''' ORDER BY t.id), '''')) FROM archive.%I t',
      backup_table
    ...
    INSERT INTO archive.progression_backup_manifest (
      ...
      'progression_20260824t024900z', 'ygyiygqfiupadgnaaxlz', 'public', source_table,
      'archive', backup_table, source_count, backup_count, source_hash, backup_hash,
      'Restore only in a controlled transaction, preserving IDs, foreign keys and unrelated records.'
    );
  END LOOP;
END $$;
...
WHERE backup_id = 'progression_20260824t024900z'
GROUP BY backup_id;
